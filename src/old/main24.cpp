    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          // stack the deck against the outer lane if car_y > 2250
          if (car_y > 2250 && lanes[1].size() > 0 && lanes[2].size() > 0 && lanes[1][0] < lanes[2][0]) {
            lanes[2][0] = lanes[1][0];
          }
          ...
          for (int lane = 0; lane<3; lane++) {
            ...
            if (lanes[lane].size() > 0) {
              ...
              if (lanes[bestlane].size() > 0 && (lanes[bestlane][0] <= lanes[lane][0]) ) {
                ...
                if (lanes[ourlane].size() > 0 && lanes[ourlane][0] < 80. && abs(ourlane-lane)==1) {
                  // and is not the outerlane when car_y > 2250
                  if (abs(ourlane-lane) == 1 && (car_y <= 2250. || lane != 2)) {
                    bestlane = lane;
                  } else {
            ...
            // if the lane is cleared of vehicles
              ...
              if (lanes[ourlane].size() > 0 && lanes[ourlane][0] < 80. && lanes[bestlane].size() > 0 && abs(ourlane-lane)==1) {
                // prevent the car to go to the outer lane when car_y > 2250 to avoid out of lane bug.
                if (car_y > 2250 && lane != 2) {
                  if (abs(ourlane-lane) == 1 && (car_y > 2250 && lane != 2)) {
                    bestlane = lane;
                  } else {
                    if (lanes[1].size() > 1 && lanes[1][0] > 14) {
                      bestlane = lane;
                    }
                  }
                  if (dist_inc < inc_max) {
                ...
                // no restriction if the car_y <= 2250
                // and move the car from the outer lane when car_y > 2250 to avoid out of lane bug.
                if (car_y <= 2250 || ourlane == 2) {
          ...
          if (lanes[ourlane].size() > 0 && lanes[ourlane][0] < 40.) {
            ...
            for (int i=0; i<sensor_fusion.size(); i++) {
              ...
              if (vid[7] == lanes[ourlane][0]) {
                ...
                if (vid[8] > 0.1) {
                  if (car_y > 2250. && ourlane == 2) {
                    // slow down even more - we need to move out of this lane!
                    dist_inc = vid[8]*0.8;
                  } else {
                    dist_inc = vid[8];
                  }
