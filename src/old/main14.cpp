        ...
        if (event == "telemetry") {
          ...
          if (timestep > 50 && not lanechange) {
            ...
            for (int k = 0; k<sensor_fusion.size(); k++) {
              ...
              double vidx = double(vid[1])+double(vid[3])*0.1;
              double vidy = double(vid[2])+double(vid[4])*0.1;
            ...
            for (int lane = 0; lane<3; lane++) {
              ...
              if (lanes[lane].size() > 0) {
                ...
                if (lanes[bestlane].size() > 0 && (lanes[bestlane][0] < lanes[lane][0])) {
                  // only switch if ourlane has a vehicle less than 60 meters away.
              ...
              // if the lane is cleared of vehicles
                ...
                // only switch if ourlane has a vehicle less than 60 meters away and is next lane over.
            ...
            if (ourlane != bestlane && car_speed > 30.0) {
              ...
              if (lanes[ourlane].size() > 0 && lanes[ourlane][0] < 15.) {
                dist_inc = 0.6*inc_max;
              } else {
                if (lanes[ourlane].size() > 0 && lanes[ourlane][0] < 30.) {

                  // slow down more - the other vehicle is coming nearer
                  dist_inc = 0.8*inc_max;
                } else {
