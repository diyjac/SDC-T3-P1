...
int main() {
  ...
  h.onMessage([&map_waypoints_x,&map_waypoints_y,&map_waypoints_s,&map_waypoints_dx,&map_waypoints_dy,&vx,&vy,&vd,&xyd,&nextd,&dist_inc,&out_log,&timestep,&lanechange](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length,
                     uWS::OpCode opCode) {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          if (path_size == 0)
          {
            ...
            for (int i = 0; i<num_points; i++)
            {
              ...
              if (i > 0)
                ...
                vd.push_back(0.);
              vector<double> xy2 = getXY(nexts, nextd, map_waypoints_s, map_waypoints_x, map_waypoints_y);
          ...
          else
          {
            ...
            t2.push_back(double(20));
            t2.push_back(double(num_points));
            nd.push_back(vy[0]);
            nd.push_back(vy[20]);
            nd.push_back(nextd);
            s3.set_points(t2,nd);
            ...
            for (int i = path_size; i<num_points; i++)
            {
              ...
              vx.push_back(nexts);
              // vy.push_back(frenet[1]);
              vy.push_back(s3(double(i)));
              if (i > 0)
                vd.push_back(distance(vx[i-1], vy[i-1], vx[i], vy[i]));
              else
                vd.push_back(0.);
              vector<double> xy2 = getXY(vx[i], vy[i], map_waypoints_s, map_waypoints_x, map_waypoints_y);
              next_x_vals.push_back(xy2[0]);
              next_y_vals.push_back(xy2[1]);
            }
          }

          if (timestep > 100 && not lanechange)
          {
