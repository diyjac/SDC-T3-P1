...
int main() {
  ...
  out_log << "t,x,y,vd,xyd,nd,d" << endl;
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          if (path_size == 0)
          {
            ...
            out_log << timestep << "," << car_x << "," << car_y << ","  << "0,0," << nextd << "," << frenet[1] << std::endl;
          ...
          // we are already moving...
            ...
            for (int i = 0; i < (num_points-path_size); i++)
            {
              ...
              out_log << timestep << "," << vx[i] << "," << vy[i] << "," << vd[i] << "," << xyd[i] << "," << nextd << "," << frenet[1] << std::endl;
            }
            ...
            std::cout << "x,y: " << car_x << "," << car_y << " localy=[" << smooth_lanes(0.) << "], frenet_d=[" << frenet[1] << "]" << std::endl;
          ...
          // don't make any plans for the first step
          // need to get the trajectory out to get the vehicle in motion.
          if (timestep > 0) {
            vector<double> lane1;
            vector<double> lane2;
            vector<double> lane3;
            ...
            for (int k = 0; k<sensor_fusion.size(); k++) {
              ...
              sensor_fusion[k].push_back(distance(0,0,vid[3],vid[4])*0.0195);
            ...
            for (int lane = 0; lane<3; lane++) {
              ...
              if (lanes[lane].size() > 0) {
                ...
                if (lanes[bestlane].size() > 0 && (lanes[bestlane][0] < lanes[lane][0])) {
                  // only switch if ourlane has a vehicle less than 80 meters away.
                  if (lanes[ourlane].size() > 0 && lanes[ourlane][0] < 80. && abs(ourlane-lane) == 1) {
                    if (abs(ourlane-lane) == 1) {
                      // and it is more than 30 meters away.
                      if (lanes[lane][0] > 30) {
                        bestlane = lane;
                      }
                      if (dist_inc < inc_max) {
            ...
            if (lanes[ourlane].size() > 0 && lanes[ourlane][0] < 40.) {
              ...
              for (int i=0; i<sensor_fusion.size(); i++) {
                ...
                if (vid[7] == lanes[ourlane][0]) {
                  ...
                  if (vid[8] > 0.1) {
                    if (dist_inc > vid[8]) {
                      // vehicle is slower than us... gradural decelerate to its speed
                      dist_inc = vid[8];
                      cout << "decelerating: setting speed spacing to: " << dist_inc << endl;
                    } else {
                      // vehicle is faster than us... gradural accelerate to its speed
                      dist_inc = dist_inc + (vid[8] - dist_inc)/3.;
