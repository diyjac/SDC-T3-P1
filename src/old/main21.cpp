...
int main() {
  ...
  h.onMessage([&map_waypoints_x,&map_waypoints_y,&map_waypoints_dx,&map_waypoints_dy,&vx,&vy,&vd,&xyd,&nld,&nextd,&inc_max,&dist_inc,&out_log,&timestep,&lanechange]
        ...
        if (event == "telemetry") {
          ...
          // we are already moving...
            ...
            // if (path_size > 0) {
              ...
              {
                out_log << timestep << "," << vx[i] << "," << vy[i] << "," << vd[i] << "," << xyd[i] << "," << nld[i] << std::endl;
              }
              ...
              nld.erase(nld.begin(),nld.begin()+(num_points-path_size));

              // if we are changing lanes
              if (lanechange && abs(nld[0]-nextd) < 0.1) {
                lanechange = false;
              }
              if (not lanechange) {
                // set up lane tracking using spline
            ...
            for(int i = path_size; i<num_points; i++) {
              ...
              if (lanechange) {
                // set up lane change tracking using spline
                vector<vector<double>> localwxy = getLocalizedWayPointSegement(car_x, car_y, car_yaw, smooth_lanechange(double(i)), map_waypoints_x, map_waypoints_y, map_waypoints_dx, map_waypoints_dy);
                // wrong way!
                if (localwxy[0][0] > 0.) {
                  car_yaw += 180;
                  cout << "wrong direction detected! car x,y,yaw: " << car_x << "," << car_y << "," << car_yaw-180 << " new yaw: " << car_yaw << endl;
                  localwxy = getLocalizedWayPointSegement(car_x, car_y, car_yaw, smooth_lanechange(double(i)), map_waypoints_x, map_waypoints_y, map_waypoints_dx, map_waypoints_dy);
                }
                smooth_lanes.set_points(localwxy[0], localwxy[1]);
              }
              ...
              vx.push_back(0.0);
              vy.push_back(0.0);
              nld.push_back(smooth_lanechange(double(i)));
