      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          else
          {
            ...
            t2.push_back(double(num_points*1.7));
            nd.push_back(vy[0]);
          ...
          if (timestep > 50 && not lanechange) {
            ...
            for (int k = 0; k<sensor_fusion.size(); k++) {
              ...
              double vidx = double(vid[1])+double(vid[3])*0.2;
              double vidy = double(vid[2])+double(vid[4])*0.2;
              vector<double> vidlocal = getLocalXY(car_x, car_y, deg2rad(car_yaw), vidx, vidy);
              double viddist = distance(car_x, car_y, double(vid[1]), double(vid[2]));
              double vids = vidlocal[0] + 5; // double(vid[5]) - ourcarsd[0] + 10;
              double vidd = double(vid[6]);
            ...
            for (int lane = 0; lane<3; lane++) {
              ...
              if (lanes[lane].size() > 0) {
                ...
                if (lanes[bestlane].size() > 0 && (lanes[bestlane][0] < lanes[lane][0])) {
                  // only switch if ourlane has a vehicle less than 50 meters away and is the next lane over.
                  if (lanes[ourlane].size() > 0 && lanes[ourlane][0] < 50. && abs(ourlane-lane)==1) {
                    bestlane = lane;
                    dist_inc = smooth_speed(path_size);
                  }
                }
              // if the lane is cleared of vehicles
              } else {
                // only switch if ourlane has a vehicle less than 50 meters away and is next lane over.
                if (lanes[ourlane].size() > 0 && lanes[ourlane][0] < 50. && lanes[bestlane].size() > 0 && abs(ourlane-lane)==1) {
                  bestlane = lane;
                  dist_inc = smooth_speed(path_size);
                }
              }
            ...
            cout << "lane0:" << lane0size << ":" << lane0closest << " lane1:" << lane1size << ":" << lane1closest << " lane2:" << lane2size << ":" << lane2closest << " ourlane:" << ourlane << endl;
            if (ourlane != bestlane) {
              cout << "ourlane:" << ourlane << " bestlane:" << bestlane << endl;
              nextd = double(2+bestlane*4) - 0.1;
              lanechange = true;
            } else {
              // no good way out - slow way down - the other vehicle is too near
              if (lanes[ourlane].size() > 0 && lanes[ourlane][0] < 15.) {
                dist_inc = 0.5*inc_max;
              } else {
                if (lanes[ourlane].size() > 0 && lanes[ourlane][0] < 25.) {

                  // slow down more - the other vehicle is coming nearer
                  dist_inc = 0.75*inc_max;
                } else {
 
                  // slow down - the other vehicle is coming near
                  if (lanes[ourlane].size() > 0 && lanes[ourlane][0] < 50.) {
                    dist_inc = 0.9*inc_max;

                  // no vehicle - set max speed.
                  } else {
                    dist_inc = inc_max;
                  }
                }
