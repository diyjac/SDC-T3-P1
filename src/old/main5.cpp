...
int main() {
  ...
  h.onMessage([&map_waypoints_x,&map_waypoints_y,&map_waypoints_s,&map_waypoints_dx,&map_waypoints_dy,&vx,&vy,&vd,&xyd,&out_log](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length,
                     uWS::OpCode opCode) {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          else
          {
            ...
            // check last 5 path points are within reason...
            if (path_size > 5)
            {
              for (int i = path_size-5; i < path_size; i++)
              {
                if (distance(vx[i-1], vy[i-1], vx[i], vy[i]) > dist_inc)
                {
                  vector<double> xy = getXY(vx[i], vy[i], map_waypoints_s, map_waypoints_x, map_waypoints_y);
                  cout << "!!!REMOVED: " << vx[i] << "," << vy[i] << "," << vd[i] << "," << xy[0] << "," << xy[1] << "," << xyd[i] << std::endl;
                  vx.erase(vx.begin()+i,vx.begin()+i+1);
                  vy.erase(vy.begin()+i,vy.begin()+i+1);
                  vd.erase(vd.begin()+i,vd.begin()+i+1);
                  xyd.erase(xyd.begin()+i,xyd.begin()+i+1);
                  path_size--;
                }
              }
            }
          ...
          for(int i = 0; i < num_points; i++)
          {
            ...
            cout << i << " x0,y0: " << localxx << "," << localxy << " x1,y1: " << localx[i] << "," << localy[i] << " dist: " << dist << endl;
            localxx = localx[i];
            localxy = localy[i];
          }
