      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          else
          {
            ...
            t2.push_back(double(num_points*1.55));
            nd.push_back(vy[0]);
          ...
          if (timestep > 50 && not lanechange) {
            ...
            for (int k = 0; k<sensor_fusion.size(); k++) {
              ...
              vector<double> vidlocal = getLocalXY(car_x, car_y, deg2rad(car_yaw), double(vid[1]), double(vid[2]));
            ...
            for (int lane = 0; lane<3; lane++) {
              ...
              if (lanes[lane].size() > 0) {
                ...
                if (lanes[bestlane].size() > 0 && (lanes[bestlane][0] < lanes[lane][0])) {
                  // only switch if ourlane has a vehicle less than 50 meters away.
                  if (lanes[ourlane].size() > 0 && lanes[ourlane][0] < 50. && abs(lane-ourlane)==1) {
                    bestlane = lane;
                  }
                }
              // if the lane is cleared of vehicles
                ...
                if (lanes[ourlane].size() > 0 && lanes[ourlane][0] < 50. && lanes[bestlane].size() > 0) {
                  bestlane = lane;
                }
              }
            }
