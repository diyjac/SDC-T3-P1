...
vector<vector<double>> getLocalizedWayPointSegement(double car_x, double car_y, double car_yaw, int lane, vector<double> maps_x, vector<double> maps_y, vector<double> maps_dx, vector<double> maps_dy) {
  vector<double> wpx;
  vector<double> wpy;
...
int main() {
  ...
  out_log << "t,x,y,vd,xyd" << endl;

  h.onMessage([&map_waypoints_x,&map_waypoints_y,&map_waypoints_dx,&map_waypoints_dy,&vx,&vy,&vd,&xyd,&curlane,&tarlane,&lane_t,&inc_max,&dist_inc,&out_log,&timestep,&lanechange]
    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          vector<tk::spline> smooth_lanes;

          tk::spline smooth_local_lane0;
          tk::spline smooth_local_lane1;
          tk::spline smooth_local_lane2;

          tk::spline smooth_speed;
          tk::spline smooth_local;

          // set up lane tracking using spline
          vector<vector<double>> localw0xy = getLocalizedWayPointSegement(car_x, car_y, car_yaw, 0, map_waypoints_x, map_waypoints_y, map_waypoints_dx, map_waypoints_dy);
          vector<vector<double>> localw1xy = getLocalizedWayPointSegement(car_x, car_y, car_yaw, 1, map_waypoints_x, map_waypoints_y, map_waypoints_dx, map_waypoints_dy);
          vector<vector<double>> localw2xy = getLocalizedWayPointSegement(car_x, car_y, car_yaw, 2, map_waypoints_x, map_waypoints_y, map_waypoints_dx, map_waypoints_dy);

          // wrong way!
          if (localw0xy[0][0] > 0.) {
            car_yaw += 180;
            cout << "wrong direction detected! car x,y,yaw: " << car_x << "," << car_y << "," << car_yaw-180 << " new yaw: " << car_yaw << endl;
            localw0xy = getLocalizedWayPointSegement(car_x, car_y, car_yaw, 0, map_waypoints_x, map_waypoints_y, map_waypoints_dx, map_waypoints_dy);
            localw1xy = getLocalizedWayPointSegement(car_x, car_y, car_yaw, 1, map_waypoints_x, map_waypoints_y, map_waypoints_dx, map_waypoints_dy);
            localw2xy = getLocalizedWayPointSegement(car_x, car_y, car_yaw, 2, map_waypoints_x, map_waypoints_y, map_waypoints_dx, map_waypoints_dy);
          }

          smooth_local_lane0.set_points(localw0xy[0], localw0xy[1]);
          smooth_local_lane1.set_points(localw1xy[0], localw1xy[1]);
          smooth_local_lane2.set_points(localw2xy[0], localw2xy[1]);
          smooth_lanes.push_back(smooth_local_lane0);
          smooth_lanes.push_back(smooth_local_lane1);
          smooth_lanes.push_back(smooth_local_lane2);
          ...
          if (path_size == 0)
          {
            ...
            for (int i = 0; i<num_points; i++)
            {
              ...
              nextlwpy = smooth_lanes[curlane](nextlwpx);
            ...
            for(int i = 0; i < num_points; i++)
            {
              ly[i] = smooth_lanes[curlane](lx[i]);
            ...
            out_log << timestep << "," << car_x << "," << car_y << ","  << "0,0" << std::endl;
          ...
          // we are already moving...
            ...
            if (path_size > 0) {
              ...
              {
                out_log << timestep << "," << vx[i] << "," << vy[i] << "," << vd[i] << "," << xyd[i] << std::endl;
              }
            ...
            std::cout << "x,y: " << car_x << "," << car_y << std::endl;

            // filler
            for(int i = path_size; i<num_points; i++) {
              lx.push_back(lx[i-1]+smooth_speed(double(i)));
              if (lanechange) {
                double percent = cos(pi()*double(lane_t)/100.);
                ly.push_back(smooth_lanes[curlane](lx[i])*percent+smooth_lanes[tarlane](lx[i])*(1.-percent));
              } else {
                ly.push_back(smooth_lanes[curlane](lx[i]));
              }
              vx.push_back(0.0);
              vy.push_back(0.0);
            ...
            for(int i = 0; i < num_points; i++)
            {
              if (lanechange) {
                double percent = cos(pi()*double(lane_t)/100.);
                ly[i] = smooth_lanes[curlane](lx[i])*percent+smooth_lanes[tarlane](lx[i])*(1.-percent);
              } else {
                ly[i] = smooth_lanes[curlane](lx[i]);
              }
              double dist = distance(localxx, localxy, lx[i], ly[i]);
          ...
          for (int lane = 0; lane<3; lane++) {
            ...
            if (lanes[lane].size() > 0) {
              ...
              if (lanes[bestlane].size() > 0 && (lanes[bestlane][0] < lanes[lane][0])) {
                ...
                if (lanes[ourlane].size() > 0 && lanes[ourlane][0] < 80.) {
            ...
            // if the lane is cleared of vehicles
              ...
              if (lanes[ourlane].size() > 0 && lanes[ourlane][0] < 80. && lanes[bestlane].size() > 0) {
                if (abs(ourlane-lane) == 1) {
                  bestlane = lane;
                } else {
                  if (lanes[1].size() > 1 && lanes[1][0] > 10) {
          ...
          if (timestep > 50 && ourlane != bestlane) {
            if ( not lanechange ) {
              ...
              cout << "nextd: " << tarlane << " change lane disabled! current position: " << vx[0] << "," << vy[0] << endl;
            }
          }
